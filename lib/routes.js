 ...
 *     .then(console.log.bind(urls))
 */
const serial = funcs => funcs.reduce((promise, func) => promise.then(result => func().then(Array.prototype.concat.bind(result)), Promise.resolve([])), Promise.resolve([]));
    ...
    return new Promise((resolve, reject) => {
      openshiftRestClient().then((client) => {
        ...
        client.projects.findAll().then((projects) => {
          if (projects) {
            serial(projects.items.map((project) => () => {
              // FIXME remove hack
              // switch namespace
              privates.get(client).config.context.namespace = project.metadata.name;
              return client.routes.findAll();
            })).then((listOfRouteArrays) => {
              const routes = {};
              if (listOfRouteArrays) {
